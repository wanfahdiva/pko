...
if (isset($_POST['login'])) {
  ...
  if (strlen($nama) < 8) {
    ...
    echo "<script language='javascript'> alert('NIS Kurang Dari Delapan'); </script>";
  } else if (strlen($password) < 8) {
    # jika ada error dari kolom password yang kosong
    echo  "<script language='javascript'> alert('Password Kurang Dari Delapan'); </script>";
  } else {
    // menyeleksi data admin dengan username dan password yang sesuai
    $data1 = mysqli_query($conn, "SELECT * from siswa where nis='$nama' and password='$password'");

    // menghitung jumlah data yang ditemukan
    $cek_siswa = mysqli_num_rows($data1);
    // pengecekana
    if ($cek_siswa > 0) {
      $assosiasi = mysqli_fetch_assoc($data1);
      ...
      header("location:halaman_login?pesan=gagal");
    }
  }
}
?>
              ...
              <div class="container-fluid d-flex justify-content-center">
                <form action="" method="post" style="width: 100%;">
                  <div class="form-group">
                    ...
                    <input type="text" name="nama" id="email" class="form-control" placeholder="Masukkan NIS Anda !" autocomplete="off">
                  </div>
