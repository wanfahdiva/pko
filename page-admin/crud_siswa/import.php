...
if (isset($_POST['upload'])) {
	...
	foreach ($Reader as $Key => $Row) {
		...
		$nis = addslashes($Row[1]);
		$kelas = addslashes($Row[2]);
		$sesi = addslashes($Row[3]);
		$query = mysqli_query($conn, "INSERT INTO siswa(nama,nis,password,kelas,sesi) VALUES ('" . $nama . "', '" . $nis . "', '" . $password . "','" . $kelas . "','" . $sesi . "')");
	}
	unlink("uploads/$nama");
	if ($query) {
		header("location:../data_siswa?import=data");
	} else {
		echo "Gagal Import";
	}
}
...
<head>
	...
	<title>PKO Admin - Import Siswa</title>
							...
							<div class="card shadow mb-4">
								...
								<div class="card-body">
									<form method="post" enctype="multipart/form-data" class="form-inline">
										...
										<a class="btn btn-secondary ml-2" href="../data_siswa">Kembali</a>
									</form>
								</div>
							</div>
						</div>
